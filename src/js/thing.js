...
function render() {
    ...
    renderMap(configuration, {
        container: '#graphic .left .map',
        width: containerWidth >= 940 ? containerWidth / 2 : containerWidth,
        data: topoData,
        clipStates: CLINTON_STATES
    });

    // Render the map!
    renderMap(configuration, {
        container: '#graphic .right .map',
        width: containerWidth >= 940 ? containerWidth / 2 : containerWidth,
        data: topoData,
        clipStates: TRUMP_STATES
    });
...
var renderMap = function(typeConfig, instanceConfig) {
    ...
    // "Clip" states
    pathsElement.append('g')
        .attr('class', 'clip-states')
        .selectAll('path')
            .data(instanceConfig['data']['states']['features'])
        .enter().append('path')
            .filter(function(d) { return instanceConfig['clipStates'].includes(d['id']) })
            .attr('d', path);
